...
def retry_with_backoff(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Nếu đã hết số lần thử lại cho phép thì ném lỗi ra ngoài
            if attempt == max_retries:
                raise
            # Chờ theo cấp số nhân: 0.1s -> 0.2s -> 0.4s...
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':<30} | {'GPT-4o Response':<35} | {'Mini Response':<35} | {'4o Latency':<10} | {'Mini Latency':<12}"
    lines.append(header)
    lines.append("-" * len(header))
    for r in results:
        p = (r["prompt"][:27] + "...") if len(r["prompt"]) > 30 else r["prompt"]
        g = r["gpt4o_response"].replace("\n", " ")
        g = (g[:32] + "...") if len(g) > 35 else g
        m = r["mini_response"].replace("\n", " ")
        m = (m[:32] + "...") if len(m) > 35 else m
        
        gl = f"{r['gpt4o_latency']:.2f}s"
        ml = f"{r['mini_latency']:.2f}s"
        lines.append(f"{p:<30} | {g:<35} | {m:<35} | {gl:<10} | {ml:<12}")
    return "\n".join(lines)
